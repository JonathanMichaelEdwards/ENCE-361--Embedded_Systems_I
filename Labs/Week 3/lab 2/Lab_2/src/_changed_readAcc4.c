...
main (void)
{
    ...
    float _accelScale = G * 2.0f/8192.0f; // ?setting the accel scale to 2G  // 13 bit

    vector3_t acc_raw = getAcclData();

    int16_t x_start = acc_raw.x;
    int16_t y_start = acc_raw.y;
    int16_t z_start = acc_raw.z;
    ...
    while (1)
    {
        ...
        int16_t _axcounts = acceleration_raw.x;
        int16_t _aycounts = acceleration_raw.y;
        int16_t _azcounts = acceleration_raw.z;

//        acceleration_raw.x * 9180 / acceleration_raw.x
        // starting z axis = 218
        int16_t _ax = (((float)_axcounts * G) / z_start);
        int16_t _ay = (((float)_aycounts * G) / z_start);
        int16_t _az = ((((float)_azcounts * G) / z_start) - G);
...
//        int16_t _az = (((float)(tZ[0]*_axcounts + tZ[1]*_aycounts + tZ[2]*_azcounts) * _accelScale));// - _azb)*_azs;
        ...
        // Find polar angles
        // z changes
        float thetaX = acos(x_start / (_ax * sin(_ay)));
        float thetaY = acos(y_start / (_ax * sin(_ay)));
        float thetaZ = acos(z_start / (_ax * sin(_ay)));

        // Raw Data
        displayUpdate ("Accl", "theta X", (int16_t)thetaX, 1);
        displayUpdate ("Accl", "theta Y", (int16_t)thetaY, 2);
        displayUpdate ("Accl", "theta Z", (int16_t)thetaZ, 3);
